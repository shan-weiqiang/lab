...
class MemorySourceTree : public google::protobuf::compiler::SourceTree {
 public:
  ...
  google::protobuf::io::ZeroCopyInputStream* Open(
    ...
    if (filename != filename_) return nullptr;
...
int main() {
  ...
  // Test: Create a new dynamic message from the original descriptor
  cout << "--- Testing New Dynamic Message from Original Descriptor ---\n";
  google::protobuf::DynamicMessageFactory original_factory(&pool);
  google::protobuf::Message* original_message_dyn =
      original_factory.GetPrototype(message_desc)->New();

  if (original_message_dyn) {
    cout << "Successfully created new dynamic message from original "
            "descriptor.\n";
    cout << "Original message type: " << original_message_dyn->GetTypeName()
         << "\n";

    const google::protobuf::Reflection* original_reflection =
        original_message_dyn->GetReflection();
    const google::protobuf::FieldDescriptor* original_my_field =
        message_desc->FindFieldByName("my_field");
    const google::protobuf::FieldDescriptor* original_another_field =
        message_desc->FindFieldByName("another_field");

    cout << "Can access 'my_field' in original message? "
         << (original_my_field ? "YES" : "NO") << "\n";
    // Still can not access the new-added field, hence it's immutable. Its
    // because we only added the field to to *Proto object, which is still not
    // built. But we can not re-build it because file with same name has already
    // been built! This means that we can not change a type after built.
    cout << "Can access 'another_field' in original message? "
         << (original_another_field ? "YES" : "NO") << "\n";

    delete original_message_dyn;
  } else {
    cout << "Failed to create new dynamic message from original descriptor.\n";
  }
  cout << "\n";
